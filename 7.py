...
def banner():
    ...
    print("=" * 60)
    print(colored("       Tool treo sớ + tag đa cookie by Cte Vcl 🧸", 'white', attrs=['bold']))
    print("=" * 60)
...
def get_fb_dtsg_jazoest(cookie, target_id):
    """Lấy fb_dtsg và jazoest từ cookie (giống nhaytagmessthat.py)"""
    try:
...
class Mention:
    """Class tạo mention tag thật (giống nhaytagmessthat.py)"""
...
def send_message_with_tags_real(idbox, fb_dtsg, jazoest, cookie, message_body, tag_ids, tag_names):
    """Gửi tin nhắn với tag thật (giống nhaytagmessthat.py)"""
    try:
        uid = get_uid(cookie)
        ...
        print(f'Lỗi gửi tin nhắn với tag thật tới ID {idbox}: {e}')
        return False
...
class MessageSender:
    ...
    def send_message_with_tag(self, text=None, thread_id=None, tag_uid=None, tag_name=None):
        """Gửi tin nhắn với tag thật sử dụng phương thức POST"""
        if text is None or thread_id is None:
            return False
        
        try:
            fb_dtsg, jazoest = get_fb_dtsg_jazoest(self.dataFB['cookieFacebook'], thread_id)
            if not fb_dtsg or not jazoest:
                print(colored("Không thể lấy fb_dtsg/jazoest cho tag thật", 'red'))
                return False
            
            # Gửi tin nhắn với tag thật
            success = send_message_with_tags_real(
                idbox=thread_id,
                fb_dtsg=fb_dtsg,
                jazoest=jazoest,
                cookie=self.dataFB['cookieFacebook'],
                message_body=text,
                tag_ids=[tag_uid] if tag_uid else [],
                tag_names=[tag_name] if tag_name else []
            )
            
            return success
        except Exception as e:
            print(colored(f"Lỗi khi gửi tin nhắn với tag thật: {e}", 'red'))
            return False

    def send_message_with_multiple_tags_real(self, text=None, thread_id=None, tag_items=None):
        """Gửi tin nhắn với nhiều tag thật"""
        if text is None or thread_id is None or not tag_items:
            return False
        ...
        try:
            ...
            tag_ids = [item["uid"] for item in tag_items]
            ...
            print(colored(f"Lỗi khi gửi tin nhắn với nhiều tag thật: {e}", 'red'))
            return False
...
def run_cookie_thread(cookie, id_list, valid_files, delay, user_info, tag_info, thread_num, all_threads):
    try:
        ...
        if tag_info.get("multiple_tags"):
            ...
            print(colored(f"[✓] Thread {thread_num}: Đang tag thật {tag_display} ({tag_count} người)", 'cyan'))
        elif tag_info["uid"]:
            print(colored(f"[✓] Thread {thread_num}: Đang tag thật {tag_info['name']} ({tag_info['uid']})", 'cyan'))
        ...
        while sender.is_running:
            try:
                for thread_id in id_list:
                    ...
                    # Fake typing (chỉ hiển thị, không ảnh hưởng tới tag thật)
                    ...
                    if tag_info.get("multiple_tags"):
                        ...
                        if tag_count <= 3:
                            ...
                            tag_display = f"Tag thật {', '.join(tag_names)}"
                        else:
                            tag_display = f"Tag thật {tag_count} người"
                        
                        print(colored(f"[{timestamp}] Thread {thread_num} -> Box {thread_id} | {tag_display}", 'cyan'))
                        print(colored(f"    Tin nhắn: {message[:50]}...", 'white'))
                        
                        # Gửi tin nhắn với nhiều tag thật
                        success = sender.send_message_with_multiple_tags_real(
                            text=message,
                            thread_id=thread_id,
                            tag_items=tag_info["multiple_tags"]
                        )
                        
                        if not success:
                            print(colored(f"[!] Thread {thread_num}: Lỗi gửi tin nhắn với tag thật", 'red'))
                    
                    elif tag_info["uid"]:
                        # 1 tag thật
                        print(colored(f"[{timestamp}] Thread {thread_num} -> Box {thread_id} | Tag thật: {tag_info['name']}", 'cyan'))
                        print(colored(f"    Tin nhắn: {message[:50]}...", 'white'))
                        
                        # Gửi tin nhắn với tag thật
                        success = sender.send_message_with_tag(
                            text=message, 
                            thread_id=thread_id,
                        ...
                        # Không tag - gửi tin nhắn thông thường qua MQTT
                        print(colored(f"[{timestamp}] Thread {thread_num} -> Box {thread_id} | Không tag", 'cyan'))
                        print(colored(f"    Tin nhắn: {message[:50]}...", 'white'))
                        
                        # Gửi tin nhắn không tag qua MQTT
                        success = sender.send_message(message, thread_id)
                        
                        if not success:
                            print(colored(f"[!] Thread {thread_num}: Lỗi gửi tin nhắn", 'red'))
    ...
    try:        
        ...
        for i, (cookie, info, tag_info) in enumerate(zip(cookie_list, user_info_list, tag_info_list), 1):
            ...
            if tag_info.get("multiple_tags"):
                ...
                tag_display = f"Tag thật: {tag_info['name']}"
            else:
                tag_display = "Không tag"
        ...
        print(colored("[*] Đang chạy với TAG THẬT...", 'yellow'))
        print(colored("[*] Nhấn Ctrl+C để dừng", 'red', attrs=['bold']))
